...
import Bestie_Robot.modules.sql.log_channel_sql as logsql
...
async def send(update, message, keyboard, backup_message):
    ...
    if cleanserv:
        with contextlib.suppress(BadRequest):
            await dispatcher.bot.delete_message(int(chat.id), update.message.message_id)
        reply = False
    try:
...
WELC_HELP_TXT = (
    ...
    "buttons.\n"
    f"To create a button linking to your rules, use this: `[Rules](buttonurl://t.me/Bestie_Robot?start=group_id)`. "
...
dispatcher.add_handler(MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS, new_member, block=False))
dispatcher.add_handler(MessageHandler(filters.StatusUpdate.LEFT_CHAT_MEMBER, left_member, block=False))
dispatcher.add_handler(CommandHandler("welcome", welcome, filters=filters.ChatType.GROUPS))
dispatcher.add_handler(CommandHandler("goodbye", goodbye, filters=filters.ChatType.GROUPS))
dispatcher.add_handler(CommandHandler("setwelcome", set_welcome, filters=filters.ChatType.GROUPS))
dispatcher.add_handler(CommandHandler("setgoodbye", set_goodbye, filters=filters.ChatType.GROUPS))
dispatcher.add_handler(CommandHandler("resetwelcome", reset_welcome, filters=filters.ChatType.GROUPS))
dispatcher.add_handler(CommandHandler("resetgoodbye", reset_goodbye, filters=filters.ChatType.GROUPS))
dispatcher.add_handler(CommandHandler("welcomemute", welcomemute, filters=filters.ChatType.GROUPS))
dispatcher.add_handler(CommandHandler("cleanservice", cleanservice, filters=filters.ChatType.GROUPS))
dispatcher.add_handler(CommandHandler("cleanwelcome", clean_welcome, filters=filters.ChatType.GROUPS))
dispatcher.add_handler(CommandHandler("welcomehelp", welcome_help))
dispatcher.add_handler(CommandHandler("welcomemutehelp", welcome_mute_help))
dispatcher.add_handler(CallbackQueryHandler(user_button, pattern=r"user_join_", block=False))
dispatcher.add_handler(CallbackQueryHandler(user_captcha_button, pattern=r"user_captchajoin_\([\d\-]+,\d+\)_\(\d{4}\)", block=False))
